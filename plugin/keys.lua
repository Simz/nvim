...
wk.add(
    {
        ...
        --{ "<leader>p",  "<cmd>lua require('fzf-lua').files({ rg_opts = '--column --line-number --no-heading --color=always --smart-case --max-columns=4096 --e --no-ignore-vcs --hidden --files' })<CR>", desc = "Fzf all files" },
        { "<leader>p",  "<cmd>lua require('fzf-lua').files({ fd_opts = '--standard --no-ignore --hidden' })<CR>", desc = "Fzf all files" },
